...
struct node * swap_adjacent(struct node **);
...
//	struct node *ret;	
	...
	printf("  Remove duplicate node   : press 11  \n");
	printf("  loop detect             : press 12  \n");
	printf("Swap two Adjcent node     : press 14  \n");
	scanf("%d",&no);
	switch(no)
	{
		...
		case DELETE_FIRST:	delete_first(start);
					break;
		case SWAP_ADJACENT:	swap_adjacent(&start);
					break;
...
struct node * swap_adjacent(struct node **start)
{
	...
	if (current != NULL)
	{
		...
		temp = current->next;
		printf("temp = current->next ::  %p\n",current->next);
		printf("prev = %p\n",prev);
		current->next = prev;
		printf("current->next <<<<= prev : %p\n",current->next);
		printf("current = %p\n",current);
		prev->next = temp;	
		printf("prev->next  <<<<= temp = %p\n",prev->next);
		print(current);	
		current = temp;
	}
