...
void insert(struct node **start, void *new_data, int data_size)
{
	int i;
...
{	
	...
	switch(no)
	{
		case 1:		//data_int(printInt);
				insert(&start, data_int, int_size);
				break;
		case 2:		insert(&start, &arr2[i], float_size);
				break;
