...
static int __init gpio_init(void){
	...
	result = request_irq(irqno, gpio_irq_handler, IRQF_TRIGGER_RISING, "gpio_handler", NULL);      //  It will enable a given interrupt line for handling the handler function with request_irq(). basically it will register an interrupt handler. 	
...
irq_handler_t gpio_irq_handler(unsigned irq, void *dev_id, struct pt_regs *regs){
	...
	return (irq_handler_t)IRQ_HANDLED;  // Announce that IRQ handled properly
}
